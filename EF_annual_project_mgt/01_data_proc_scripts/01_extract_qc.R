...
data <- dbf_io(file_path_in = "./data/dbf/123a_1") %>%     
...
fish_tmp <- map_df(data[grepl("fish", names(data))], bind_rows) %>% 
  ...
  mutate(ray_ct = na_if(ray_ct, "N"),
         tubercles = ifelse(species %in% native, tubercles, NA),        # Cleans up additional vars
...
site <- site_tmp %>% 
  ...
  left_join(rvr_join, by = c("reach" = "rch_code")) %>%                   # Add rvr_abbr variable
...
dir.create(paste0("./output/raw_csv/", data_id))
write_csv(meta, paste0("./output/raw_csv/", data_id, "/raw_meta.csv"), na = "")
write_csv(ck_site, paste0("./output/raw_csv/", data_id, "/raw_site.csv"), na = "")
write_csv(ck_fish, paste0("./output/raw_csv/", data_id, "/raw_fish.csv"), na = "")
write_csv(ck_pit, paste0("./output/raw_csv/", data_id, "/raw_pit.csv"), na = "")
write_csv(ck_floy,paste0("./output/raw_csv/", data_id, "/raw_floy.csv"), na = "")
write_csv(water, paste0("./output/raw_csv/", data_id, "/raw_water.csv"), na = "")
